...
async function fetchAndSaveExteriorColors(year) {
  const baseUrl = "https://carapi.app/api/exterior-colors";
  ...
  try {
    while (url) {
      ...
      for (const color of data) {
        ...
        const insertQuery = `
            INSERT INTO make_model_trim_exterior_colors (
    ...
    console.log(
      `Total exterior colors saved for year ${year}: ${totalColorsSaved}`
    );
  } catch (error) {
    console.error(
      `Error in fetchAndSaveExteriorColors for year ${year}:`,
      error
    );
...
async function fetchAndSaveExteriorColorsForYears() {
  for (let year = 2015; year <= 2020; year++) {
    await fetchAndSaveExteriorColors(year);
  }
}

fetchAndSaveExteriorColorsForYears();
